...
class ExtensionRegistrationViaParametersAndFieldsTests extends AbstractJupiterTestEngineTests {
	...
	private List<String> getRegisteredExtensions(LogRecordListener listener) {
		String packagePrefix = "[org.junit.jupiter.engine.extension.";

		// @formatter:off
		return listener.stream(MutableExtensionRegistry.class, Level.FINER)
			.map(LogRecord::getMessage)
			.filter(message -> message.contains("local extension") && message.contains(packagePrefix))
			.map(message -> message.substring(message.indexOf(packagePrefix) + packagePrefix.length(), message.indexOf("$")))
			.collect(toList());
		// @formatter:on
	}
